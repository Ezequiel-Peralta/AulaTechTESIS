<?php
$teacher_aide_info = $this->TeachersAide_model->get_teacher_aide_info($param2);
    ...
    <div class="profile-details card mt-4" style="background-color: #fff; border-radius: 15px; padding: 10px 20px 20px 20px;">
        <div class="card-body">
            ...
            <div class="profile-container">
                <?php
                    // Buscar secciones asociadas al teacher_aide_id en la tabla section
                    $this->db->where('teacher_aide_id', $row['teacher_aide_id']);
                    $sections_teacher_aide = $this->db->get('section')->result_array();

                    // Si no encuentra registros, buscar en section_history
                    if (empty($sections_teacher_aide)) {
                        $this->db->where('teacher_aide_id', $row['teacher_aide_id']);
                        $sections_teacher_aide = $this->db->get('section_history')->result_array();
                    }

                    // Iterar por cada sección encontrada
                    foreach ($sections_teacher_aide as $section): ?>
                    ...
                    <?php endforeach; // Cierre del foreach ?>
            </div>
        </div>
    </div>
