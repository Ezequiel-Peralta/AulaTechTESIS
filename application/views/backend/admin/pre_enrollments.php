...
$titleEN = 'Student Report in Enrollments - ' . date('d-m-Y');
$titleES = 'Reporte de Estudiantes en Matriculación - ' . date('d-m-Y');
?>
...
<div class="row">
            ...
            <div class="tab-pane active" id="home">
                ...
                <div class="mt-2 mb-4">
                    ...
                    <div class="pull-right"> 
                        <select class="form-control customSelect text-center" id="pre_enrollment_student_bulk_btn">
                            <option value="" selected disabled><?php echo ucfirst(get_phrase('select')); ?></option>
                            <?php 
                            // Obtener el nuevo período académico activo
                            $new_academic_period = $this->db->get_where('academic_period', array('status_id' => 1))->row();
                            if ($new_academic_period) {
                        ...
                        <button id="inscription-btn-bulk" class="btn btn-secondary inscription-btn-bulk" style="display:none;">
                            <?php echo ucfirst(get_phrase('pre_enrollment'));?>
                        </button>
                    </div>
                ...
                <table class="table table-bordered datatable table-hover table-striped" id="all_student_table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center"><?php echo ucfirst(get_phrase('disable')); ?></th>
    ...
    <tr>
        ...
        <td class="text-center">
            <a href="javascript:;" onclick="showAjaxModal('<?php echo base_url();?>index.php?modal/popup/modal_inactive_student_pre_enrollments/<?php echo $row['student_id'];?>');" class="btn btn-table btn-white btn-danger-hover" title=" <?php echo ucfirst(get_phrase('disabled')); ?>">
                ...
                buttons: [
                    {
                        ...
                        title: null,
                        exportOptions: {
                            columns: ':not(:eq(0)):not(:eq(16)):not(:eq(18))' 
                        }
                    },
                    {
                        extend: 'excelHtml5',
                    ...
                    },
                    {
                        ...
                        action: function (e, dt, node, config) {
                            window.location.href = '<?php echo base_url(); ?>index.php?admin/printStudentPreEnrollmentsTableEN/';
                        }
                    }
                ],
                colReorder: true, 
       ...
       "language": {
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        className: 'btn buttons-html5 btn-white btn-sm btn-danger-hover',
                        action: function (e, dt, node, config) {
                            window.location.href = '<?php echo base_url(); ?>index.php?admin/printStudentPreEnrollmentsTableES/';
                        }
                    }
                ],
        colReorder: true, 
        ...
        $('#inscription-btn-bulk').on('click', function () {
            var selectedSectionId = $('#pre_enrollment_student_bulk_btn').val(); // Obtiene el section_id
            var selectedClassId = $('#pre_enrollment_student_bulk_btn option:selected').data('class-id'); // Obtiene el class_id asociado al section_id
...
<script>
    $(document).ready(function () {
        ...
        $('#pre_enrollment_student_bulk_btn').on('change', function () {
