<div class="mail-env">
			...
			<div class="mail-body">
				...
				<div class="mail-header">
					<!-- title -->
					<h3 class="mail-title">
						Buzón de etiqueta <?php echo ucfirst(get_phrase($active_tag)) ?>
						<?php if ($active_tag_count > 0): ?>
							<span class="count">(<?php echo $active_tag_count; ?>)</span>
						<?php else: ?>
    ...
    <?php foreach ($messages as $message): ?>
		<tr class="<?php echo $message['new_message_count'] > 0 ? 'unread' : ''; ?>">
			...
			<a href="javascript:;" style="cursor: default;">
							...
							data-content="<?php echo implode(', ', array_map('ucfirst', $message['tags'])) . '.'; ?>" 
			...
			<td class="col-options">
				...
				<?php
					$btn_class = '';
					if ($message['status'] == 'buzón') {
						$btn_class = 'btn-info-hover';
					} elseif ($message['status'] == 'papelera') {
						$btn_class = 'btn-danger-hover';
					} elseif ($message['status'] == 'archivado') {
						$btn_class = 'btn-green-hover';
					}
				?>

				<button type="button" class="btn btn-table btn-white <?php echo $btn_class; ?> popover-white" href="javascript:;"
				data-toggle="popover" data-trigger="hover" data-placement="top" data-content="Ubicación actual">
					En <?php echo $message['status'] ?>
				</button>
            </td>
        </tr>
    <?php endforeach; ?>
					...
					<?php foreach ($tag_data_map as $tag_name => $tag_info): ?>
						<li class="<?php echo $tag_info['name'] == $active_tag ? 'active' : ''; ?>">
							<a href="<?php echo base_url(); ?>index.php?admin/messages_tag/<?php echo $tag_info['name']; ?>">
								...
								<span class="tag-txt">	<?php echo ucfirst($tag_name); ?></span>
