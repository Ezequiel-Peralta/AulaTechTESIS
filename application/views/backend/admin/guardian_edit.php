<?php 
$edit_data		=	$this->Guardians_model->get_guardian_info($param2);
			...
			<div class="panel-body">
                <?php echo form_open(base_url() . 'index.php?admin/guardians/' .'update' . '/'.$row['guardian_id'] , array('class' => 'form-wizard validate', 'enctype' => 'multipart/form-data'));?>
					...
					<ul>
						...
						<li>
							<a href="#tab2-6" data-toggle="tab"><span>6</span><?php echo ucfirst(get_phrase('student_information')); ?></a>
						</li>
						<li>
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<div class="input-group">
											...
											<input type="text" class="form-control" name="username" id="username" data-validate="required" data-message-required="<?php echo ucfirst(get_phrase('required_value')); ?>" value="<?php echo $row['username'];?>"/>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="row">
                                ...
                                <div class="form-group">
										...
										<div id="student_container">
											...
											$student_guardians = $this->db->get_where('student_guardian', array('guardian_id' => $row['guardian_id']))->result_array();
											
											foreach($student_guardians as $student_guardian):
												// Obtenemos el ID, nombre y tipo de relación
												$student_id = $student_guardian['student_id'];
												$guardian_type_id = $student_guardian['guardian_type_id'];

												// Obtenemos el nombre y apellido del guardián
												$student_info = $this->db->get_where('student_details', array('student_id' => $student_id))->row_array();
												$firstname = $student_info['firstname'];
												$lastname = $student_info['lastname'];
											?>
												<div class="student-entry">
													<input type="hidden" name="existing_student_ids[]" value="<?php echo $student_id; ?>"> 
													...
													<select name="student_id[]" class="form-control">
														...
														?>
														<option value="<?php echo $row2['student_id'];?>" <?php echo ($row2['student_id'] == $student_id) ? 'selected' : ''; ?>>
													...
													<button type="button" class="btn btn-danger" onclick="removeStudentEntry(this)">-</button>
													<br>
												</div>
											...
											<div class="text-center">
												<button type="button" class="btn btn-success" onclick="addStudentEntry()" title="<?php echo ucfirst(get_phrase('add')); ?>">+</button>
											</div>
										</div>
									</div>               
								</div>
...
function addStudentEntry() {
    ...
    entry.innerHTML = `
        <select name="student_id[]" class="form-control" data-validate="required" data-message-required="<?php echo ucfirst(get_phrase('required_value')); ?>">
            <option value="" selected disabled><?php echo ucfirst(get_phrase('select')); ?></option>
            ${students.map(student => `<option value="${student.student_id}">${student.firstname} ${student.lastname}</option>`).join('')}
        </select>
        ...
        <button type="button" class="btn btn-danger" title="<?php echo ucfirst(get_phrase('remove')); ?>" onclick="removeStudentEntry(this)">-</button>
        <br>
    `;
