				...
				<div class="mail-header">
					...
					<h3 class="mail-title">
						...
						<?php if ($received_count > 0): ?>
							<span class="count">(<?php echo $received_count; ?>)</span>
						<?php else: ?>
		...
		<tr class="<?php echo $message['new_message_count'] > 0 ? 'unread' : ''; ?>">
            ...
            <td class="col-name">
                ...
                <a href="<?php echo base_url(); ?>index.php?admin/messages_read/<?php echo $message['message_thread_code']; ?>" 
					...
					data-trigger="hover" 
					data-placement="top" 
					data-content="<?php echo $message['last_sender_lastname'] . ', ' . $message['last_sender_firstname'] . '. '; ?>" 
					data-original-title="Última actividad">

						<!-- Insertar la imagen alineada a la izquierda -->
...
$(document).ready(function() {
	...
	$('#unread_message_Thread_bulk_btn').on('click', function(e) {
		...
		console.log("Seleccionando checkboxes...");
        var selectedMessageThread = [];
		...
		console.log("IDs seleccionados: ", selectedMessageThread); 
	...
	$('#add_favorite_message_Thread_bulk_btn').on('click', function(e) {
        ...
        if (selectedMessageThread.length > 0) {
            var url = '<?php echo base_url();?>index.php?admin/messages_settings/add_favorite_message_thread_bulk/messages/' + selectedMessageThread.join('-');
	...
	$('#remove_favorite_message_Thread_bulk_btn').on('click', function(e) {
        ...
        if (selectedMessageThread.length > 0) {
            var url = '<?php echo base_url();?>index.php?admin/messages_settings/remove_favorite_message_thread_bulk/messages/' + selectedMessageThread.join('-');
...
<script>
    $('#apply_filters_btn').click(async function() {
        const { value: formValues } = await Swal.fire({
            title: 'Filtros por Fecha',
            html: `
                <div>
                    <label for="dateFilter">Filtrar por:</label>
                    <select id="dateFilter" class="swal2-select" style="width: 100%">
                        <option value="select">Seleccionar</option>
                        <option value="year">Por año</option>
                        <option value="month">Por mes</option>
                        <option value="specificDay">Por día específico</option>
                        <option value="betweenDates">Entre dos fechas</option>
                    </select>
                </div>
                <div id="yearFilter" style="display:none;">
                    <label for="yearInput">Seleccionar año:</label>
                    <input type="number" min="1900" max="2099"  id="yearInput"  class="swal2-input" step="1" value="2016" />
                </div>
                <div id="monthFilter" style="display:none;">
                    <label for="monthInput">Seleccionar mes:</label>
                    <input type="month" id="monthInput" class="swal2-input" required>
                </div>
                <div id="dayFilter" style="display:none;">
                    <label for="dayInput">Seleccionar día:</label>
                    <input type="date" id="dayInput" class="swal2-input" required>
                </div>
                <div id="betweenDatesFilter" style="display:none;">
            ...
            preConfirm: () => {
                ...
                const dayInput = $('#dayInput').val();
                const startDate = $('#startDate').val();
                const endDate = $('#endDate').val();

                return { dateFilter, yearInput, monthInput, dayInput, startDate, endDate };
            },
            didOpen: () => {
                // Pre-seleccionar los valores por defecto
                const today = new Date();
                $('#yearInput').val(today.getFullYear().toString()); // Año actual
                $('#monthInput').val(today.toISOString().slice(0, 7)); // Mes actual (YYYY-MM)
                $('#dayInput').val(today.toISOString().split("T")[0]); // Día actual (YYYY-MM-DD)

                // Manejar el cambio del select de filtro
                $('#dateFilter').change(function() {
                    const selectedFilter = $(this).val();
                    
                    // Mostrar/ocultar inputs en función del filtro seleccionado
                    $('#yearFilter').toggle(selectedFilter === 'year');
                    $('#monthFilter').toggle(selectedFilter === 'month');
                    $('#dayFilter').toggle(selectedFilter === 'specificDay');
                    $('#betweenDatesFilter').toggle(selectedFilter === 'betweenDates');
                });
            }
        });
        ...
        if (formValues) {
            ...
            Swal.fire({
                title: 'Resultados de los filtros',
