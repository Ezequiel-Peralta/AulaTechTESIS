...
class Subjects extends CI_Controller
{
    ...
    function subjects_profile($subject_id = '')
    {
        ...
        $subject_info = $this->Subjects_model->get_subject_info($subject_id);
    ...
    function subjects($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $section = $this->Subjects_model->get_section($data['section_id']);
            if ($section) {
                $data['teacher_aide_id'] = $section->teacher_aide_id;
            }

            $createdSubjectId = $this->Subjects_model->create_subject($data);
            ...
            if (!empty($_FILES['userfile']['name'])) {
                ...
                $this->Subjects_model->update_subject_image($createdSubjectId, $file_name);
            } else {
                $default_image = 'assets/images/default-subject-img.jpg';
                $this->Subjects_model->update_subject_image($createdSubjectId, $default_image);
            }
        ...
        if ($param1 == 'update') {
            ...
            $current_subject_data = $this->Subjects_model->get_subject($subject_id);
            ...
            $this->Subjects_model->update_subject($subject_id, $data);
        ...
        if ($param1 == 'disable_subject') {
            ...
            if ($subject_id) {
                $this->Subjects_model->update_subject_status($subject_id, 0);
        ...
        if ($param1 == 'enable_subject') {
            ...
            if ($subject_id) {
                $this->Subjects_model->update_subject_status($subject_id, 1);
    ...
    {
        $subjects = $this->Subjects_model->get_subjects_by_class($class_id);
    ...
    {
        $subjects = $this->Subjects_model->get_subjects_by_section($section_id);
        foreach ($subjects as $row) {
            echo '<option value="' . $row['subject_id'] . '">' . $row['name'] . '</option>';
        }
    }
    ...
    function manage_subjects()
    {
        ...
        $active_sections = $this->Subjects_model->get_active_sections();
        $class_ids = array_column($active_sections, 'class_id');
        $all_classes_count = count($active_sections);

        $classes = $this->Subjects_model->get_classes_by_ids($class_ids);
    ...
    function view_subjects($section_id = '', $teacher_id = '')
    {
        ...
        if (empty($section_id)) {
            $active_academic_period = $this->Subjects_model->get_active_academic_period();

            if ($active_academic_period) {
                $active_academic_period_id = $active_academic_period->id;

                $section = $this->Subjects_model->get_first_section($active_academic_period_id);
        ...
        $section_data = $this->Subjects_model->get_section_data($section_id);

        if (empty($section_data)) {
            $section_data = $this->Subjects_model->get_section_history_data($section_id);
        ...
        $teacher_data = $this->Subjects_model->get_teacher_data($teacher_id);
    ...
    function subjects_information($section_id = '')
    {
        ...
        $all_subjects_count = $this->Subjects_model->get_section_subject_count($section_id);
