...
class Messages extends CI_Controller
{
	...
	function __construct()
	{
        ...
        $this->load->library('messages_service');
    ...
    function messages($param1 = 'message_default', $param2 = '', $param3 = '') {
        ...
        $unread_count = $this->Messages_model->get_unread_count($user_id, $user_group);
        $received_count = $this->Messages_model->get_received_count($user_id, $user_group);
        $sent_count = $this->Messages_model->get_sent_count($user_id, $user_group);
        $draft_count = $this->Messages_model->get_draft_count($user_id, $user_group);
        $trash_count = $this->Messages_model->get_trash_count($user_id, $user_group);
        $favorite_count = $this->Messages_model->get_favorite_count($user_id, $user_group);
        ...
        $breadcrumb = array(
            ...
            array(
                ...
                'url' => base_url('index.php?admin/messages')
            )
        );
    ...
    function messages_new($param1 = '', $param2 = '') {
        ...
        if ($param1 == 'send_new') {
            ...
            if (!empty($_FILES['attachments']['name'][0])) { // Verificar si hay archivos
                ...
                for ($i = 0; $i < $number_of_files; $i++) {
                    ...
                    if ($this->upload->do_upload('attachment')) {
                        ...
                        redirect(base_url() . 'index.php?admin/messages_new', 'refresh');
                    }
                }
            }
            ...
            redirect(base_url() . 'index.php?admin/messages_read/' . $message_thread_code, 'refresh');
        }
    ...
    function messages_settings($param1 = '', $param2 = '', $param3 = '', $param4 = '') {
        ...
        if ($param1 == 'favorite') {
            if ($param4 == 'add') {
                $this->Messages_model->add_favorite($param3, $user_id, $user_group);
        ...
        if ($param1 == 'draft') {
            if ($param4 == 'add') {
                $this->Messages_model->add_draft($param3, $user_id, $user_group);
                redirect(base_url() . 'index.php?admin/messages_draft/', 'refresh');
            } elseif ($param4 == 'remove') {
                $this->Messages_model->remove_draft($param3, $user_id, $user_group);
                redirect(base_url() . 'index.php?admin/messages/', 'refresh');
            }
        }
    
        if ($param1 == 'trash') {
            if ($param4 == 'add') {
                $this->Messages_model->add_trash($param3, $user_id, $user_group);
                redirect(base_url() . 'index.php?admin/messages_trash/', 'refresh');
            } elseif ($param4 == 'remove') {
                $this->Messages_model->remove_trash($param3, $user_id, $user_group);
                redirect(base_url() . 'index.php?admin/messages/', 'refresh');
            }
        }
        if ($param3 == 'trash_for_user_message_thread') {
            if ($param2 == 'add') {
                $this->Messages_model->add_trash_for_user($param1, $user_id, $user_group);
                redirect(base_url() . 'index.php?admin/messages_trash/', 'refresh');
            } elseif ($param2 == 'remove') {
                $this->Messages_model->remove_trash_for_user($param1, $user_id, $user_group);
                redirect(base_url() . 'index.php?admin/messages/', 'refresh');
            }
        }
        if ($param3 == 'trash_for_all_user_message_thread_owner') {
            if ($param2 == 'add') {
                $this->Messages_model->add_trash_for_all($param1, $user_id, $user_group);
                redirect(base_url() . 'index.php?admin/messages_trash/', 'refresh');
            } elseif ($param2 == 'remove') {
                $this->Messages_model->remove_trash_for_all($param1, $user_id, $user_group);
                redirect(base_url() . 'index.php?admin/messages/', 'refresh');
            }
        }
        ...
        if($param1 == 'user_message_status') {
            if ($param3 == 'read') {
                $this->Messages_model->mark_as_read($param2, $user_id, $user_group);
            } elseif ($param3 == 'unread') {
                $this->Messages_model->mark_as_unread($param2, $user_id, $user_group);
            }
        ...
        if ($param1 == 'move_to') {
            switch ($param3) {
                case 'draft':
                    $this->Messages_model->move_to_draft($param2, $user_id, $user_group);
                    break;
                case 'trash':
                    $this->Messages_model->move_to_trash($param2, $user_id, $user_group);
                    break;
                case 'inbox':
                    $this->Messages_model->move_to_inbox($param2, $user_id, $user_group);
                    break;
            }
            switch ($param4) {
                case 'messages_read':
        ...
        if ($param1 == 'delete_message_thread_bulk') {
            ...
            foreach ($message_thread_codes as $message_thread_code) {
                $this->Messages_model->delete_message_thread($message_thread_code, $user_id, $user_group);
            }
        ...
        if ($param1 == 'draft_message_thread_bulk') {
            ...
            foreach ($message_thread_codes as $message_thread_code) {
                $this->Messages_model->draft_message_thread($message_thread_code, $user_id, $user_group);
            }
        ...
        if ($param1 == 'read_message_thread_bulk') {
            ...
            foreach ($message_thread_codes as $message_thread_code) {
                $this->Messages_model->read_message_thread($message_thread_code, $user_id, $user_group);
            }
        ...
        if ($param1 == 'unread_message_thread_bulk') {
            ...
            foreach ($message_thread_codes as $message_thread_code) {
                $this->Messages_model->unread_message_thread($message_thread_code, $user_id, $user_group);
            }
        ...
        if ($param1 == 'add_favorite_message_thread_bulk') {
            ...
            foreach ($message_thread_codes as $message_thread_code) {
                $this->Messages_model->add_favorite_message_thread($message_thread_code, $user_id, $user_group);
            }
        ...
        if ($param1 == 'remove_favorite_message_thread_bulk') {
            ...
            foreach ($message_thread_codes as $message_thread_code) {
                $this->Messages_model->remove_favorite_message_thread($message_thread_code, $user_id, $user_group);
            }
