...
class Guardians extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->library('Guardians_service');
    ...
    {
        $guardians = $this->Guardians_model->get_guardians();

        foreach ($guardians as $row) {
            $guardian_details = $this->Guardians_model->get_guardian_info($row['guardian_id']);
    ...
    function guardian($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $insertedGuardianId = $this->Guardians_model->insert_guardian($data);
            ...
            $insertedAddressId = $this->Guardians_model->insert_address($dataAddress);
            ...
            $this->Guardians_model->insert_guardian_details($dataDetails);
            ...
            if (!empty($student_ids) && !empty($relationships)) {
                foreach ($student_ids as $index => $student_id) {
                    ...
                    $this->Guardians_model->insert_student_guardian($dataStudent);
                }
            }
        ...
        if ($param1 == 'update') {
            $guardian_id = $param2;
            $guardian_details = $this->Guardians_model->get_guardian_details($guardian_id);
            ...
            $this->Guardians_model->update_guardian($guardian_id, $data);
            ...
            $this->Guardians_model->update_address($address_id, $dataAddress);
            ...
            $this->Guardians_model->update_guardian_details($guardian_id, $dataDetails);

            $existing_student_ids = $this->input->post('existing_student_ids') ?: [];
            $student_ids = $this->input->post('student_id');
            $relationships = $this->input->post('relationship');

            $current_students = $this->Guardians_model->get_student_guardians($guardian_id);
            ...
            if (!empty($student_ids_to_delete)) {
                $this->Guardians_model->delete_student_guardians($guardian_id, $student_ids_to_delete);
            }
            ...
            foreach ($student_ids as $index => $student_id) {
                if (!in_array($student_id, $existing_student_ids)) {
                    ...
                    $this->Guardians_model->update_student_guardian($guardian_id, $student_id, $dataStudent);
                }
            }
        ...
        if ($param1 == 'disable_guardian') {
            ...
            if ($guardian_id) {
                $this->Guardians_model->update_guardian_status($guardian_id, 0);
        ...
        if ($param1 == 'enable_guardian') {
            ...
            if ($guardian_id) {
                $this->Guardians_model->update_guardian_status($guardian_id, 1);
    ...
    {
        $sections = $this->Guardians_model->get_all_guardians();
