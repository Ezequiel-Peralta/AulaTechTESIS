...
class Section extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->library('Section_service');
    ...
    function section($class_id = '')
    {
        ...
        if ($class_id == '')
            $class_id = $this->Section_model->get_first_class_id();
    ...
    function sections($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $academic_period = $this->Section_model->get_active_academic_period();
        ...
        if ($param1 == 'update') {
            ...
            $data['class_id'] = $this->input->post('class_id');
            $data['shift_id'] = $this->input->post('shift_id');
            $this->Section_model->update_section($param2, $data);
            $this->session->set_flashdata('flash_message', array(
        ...
        if ($param1 == 'disable_section') {
            ...
            if ($section_id) {
                $this->Section_model->update_section_status($section_id, 0);
        ...
        if ($param1 == 'enable_section') {
            ...
            if ($section_id) {
                $this->Section_model->update_section_status($section_id, 1);
    ...
    {
        $sections = $this->Section_model->get_all_sections();
        foreach ($sections as $row) {
            echo '<option value="' . $row['section_id'] . '">' . $row['name'] . '</option>';
        }
    }
    ...
    {
        $sections = $this->Section_model->get_sections_by_academic_period($academic_period_id);
        foreach ($sections as $row) {
    ...
    function section_routine($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $this->Section_model->insert_section_routine($data);
            $this->session->set_flashdata('flash_message', array(
        ...
        if ($param1 == 'update') {
            ...
            $this->Section_model->update_section_routine($param2, $data);
            $this->session->set_flashdata('flash_message', array(
            ...
            $page_data['edit_data'] = $this->Section_model->get_section_routine_by_id($param2);
        }
        if ($param1 == 'delete') {
            $this->Section_model->delete_section_routine($param2);
            $this->session->set_flashdata('flash_message', array(
    ...
    {
        $sections = $this->Section_model->get_active_sections();
